...
# ------------------- Background Scraper -------------------
def delayed_second_scrape(website_name, product_id):
    print(f"[INFO] Waiting 15 mins for second scrape of: {product_id}")
    ...
    if product_id in scraped_data:
        scraped_data[product_id]["second"] = second_scrape
    else:
        scraped_data[product_id] = {"second": second_scrape}
...
def choose_website(website_name, product_id):
    ...
    thread = threading.Thread(target=delayed_second_scrape, args=(website_name, product_id))
    thread.start()
...
def index():
    ...
    if request.method == 'POST':
        ...
        first_scrape = choose_website(website_name, product_id)
        return redirect(f"/?product_id={product_id}&website_name={website_name}")

    if product_id:
        data = load_data().get(product_id, {})
